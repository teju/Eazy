...
package eazi.com.eazi.materialarcmenu;
...
import eazi.com.eazi.materialarcmenu.anim.AnimationObject;
import eazi.com.eazi.materialarcmenu.anim.ViewAnim;
import eazi.com.eazi.materialarcmenu.interfaces.ScrollDirectionListener;
...
public class ArcMenu extends RelativeLayout {
	...
	private void applyAttrs(Context context, AttributeSet attrs) {
		if (attrs != null) {
			System.out.println("applyAttrs1234 called");
			...
			childRadius = b.getInt(R.styleable.ArcMenu_menuChildRadius,
					defaultChildRadius);
	...
	private OnClickListener getItemClickListener(final OnClickListener listener) {
		return new OnClickListener() {
			...
			public void onClick(final View viewClicked) {
				final int itemCount2 = mArcLayout.getChildCount();
				for (int j = 0; j < itemCount2; j++) {
					View items = mArcLayout.getChildAt(j);
					if (viewClicked != items) {
					} else {
						if (!isChild) {
							if (j == 0) {
								setMinRadius(childRadius - 100 );

								if (menuAnim && mArcLayout.isAnimDone()) {
									if (clickListener != null) {
										clickListener.onClick(fabMenu);
									}
									final int itemCount = MenuItem.ITEM_DRAWABLES2.length;
									...
									for (int i = 0; i < itemCount; i++) {
										...
										item.setImageResource(MenuItem.ITEM_DRAWABLES2[i]);
										item.setTag(i);
										addItem(item, "Hello", new OnClickListener() {

											@Override
											public void onClick(View v) {
												System.out.println("1234567890 " + position);
							...
							//Toast.makeText(mContext, "pos grater" + i, Toast.LENGTH_LONG).show();
						} else {

							if (j == 0) {
								setMinRadius(childRadius - 150 );
